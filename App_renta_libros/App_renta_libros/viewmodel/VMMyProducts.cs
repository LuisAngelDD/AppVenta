...
{
    internal class VMMyProducts : BaseViewModel
    {
        ...
        {
            this.ListViewSource = await repositoriProducto.GetAllMy(Preferences.Get("User", string.Empty));
        }
        public async Task LoadDataS(string name)
        {
            this.ListViewSource = await repositoriProducto.GetAllByName(Preferences.Get("User", string.Empty),name);
        }
        #endregion
    }
}
